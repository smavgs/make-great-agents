/**
 * Validates every file in agents/ against the contribution contract.
...
if (files.length === 0) errors.push('agents/ contains no markdown files');
...
for (const file of files) {
  ...
  if (agent.url) {
    const urlDupe = seenUrls.get(agent.url);
    if (urlDupe) errors.push(`${file}: url "${agent.url}" already used by ${urlDupe} (duplicate agent?)`);
    else seenUrls.set(agent.url, file);
  }
}
...
console.log(`✓ ${files.length} agent files valid`);
