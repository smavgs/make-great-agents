---
name: Salesforce Report Builder
category: Sales
added_at: "2026-08-17T18:23:33.000Z"
contributor: ericzakariasson
contributor_url: https://x.com/ericzakariasson
integrations: [Salesforce]
added_via: https://x.com/ericzakariasson/status/2087258914060664902
---

Set up a new agent for me I can ask for pipeline answers. Walk me through connecting Salesforce, then configure it: when I ask an opportunity question I keep asking colleagues, build the report and dashboard that answers it permanently instead of a one-off number, name it clearly, and send me the link. Ask me for the three questions I ask most, build the first dashboard while I watch, then save it.
