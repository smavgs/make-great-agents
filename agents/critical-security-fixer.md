---
name: Critical Security Fixer
category: Ops
added_at: "2026-08-23T13:11:57.000Z"
contributor: elie2222
contributor_url: https://x.com/elie2222
integrations: [Aikido, GitHub]
integration_urls:
  Aikido: https://www.aikido.dev
  GitHub: https://github.com
---

Set up a new agent for me that watches Aikido for critical security issues and opens GitHub pull requests to fix them.

Walk me through connecting Aikido and GitHub. Ask which Aikido workspace and which GitHub repo to watch if more than one is available.

When a new critical issue comes in, open a PR that fixes it. Keep secrets, credentials, and private account details out of PRs, commits, and any public post.

After the first successful check, save yourself as a scheduled agent that looks for new critical issues on weekdays and opens a PR when one appears.
